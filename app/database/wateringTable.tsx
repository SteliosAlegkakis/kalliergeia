...
export const addWatering = async (field_id: number, cost: number, cubic_meter: number, date: string, comment: string) => {
    await db.execAsync(
        `
        INSERT INTO watering (field_id, cost, cubic_meter, date, comment)
        VALUES (${field_id}, ${cost}, ${cubic_meter}, '${date}', '${comment}');
        `
    );
}
...
export const getWateringCostTotal = async () => {
    const watering = await db.getAllAsync(
        `
        SELECT IFNULL(SUM(cost), 0) as totalCost FROM watering;
        `
    );
    return watering;
}
